...
import { Check, ChevronDown, Sun, X } from "lucide-react";
...
export function OnboardingPhotoGuide() {
  ...
  return (
    <details
      className="group rounded-xl border border-border/70 bg-muted"
      data-testid="onboarding-photo-guide"
    >
      <summary
        className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-2 px-3.5 py-2 text-sm font-semibold [&::-webkit-details-marker]:hidden"
        data-testid="onboarding-photo-guide-toggle"
      >
        <span className="min-w-0 text-pretty leading-snug">
          {t("photoGuide.collapsedLabel")}
        </span>
        <ChevronDown
